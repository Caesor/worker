...
// const arrayBuffer = wx.base64ToArrayBuffer(JSON.stringify({time: Date.now()}));

const time = Date.now();

const buf = stringToArrayBuffer(JSON.stringify({ time }));
...
const r = arrayBufferToString(buf);
console.log(r)
worker.onMessage(obj => {
  console.log(`${Date.now() - obj.time}`, '计算结果');
});
